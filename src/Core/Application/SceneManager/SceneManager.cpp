...
bool SceneManager::SaveScene(NString filename)
{
	...
	#pragma region camera
		...
		rapidjson::Value CameraSpeed(rapidjson::kObjectType);
		rapidjson::Value CameraPosArray(rapidjson::kArrayType);
	
		 for (int i = 0; i < 3; i++)
		 {
		 	rapidjson::Value temp(editorCamera->Position[i]);
		 	CameraPosArray.PushBack(temp, allocator);
		 }
	
		 rapidjson::Value CamYaw(editorCamera->Yaw);
	
		 rapidjson::Value CamPitch(editorCamera->Pitch);
	...
	#pragma components
		 ...
		 for (const auto& sceneObject : currentScene.sceneObjects)
		 {
		 	//if (/*!Entity->bIsDebug*/) {continue;}
	
		 	rapidjson::Value ObjValue(rapidjson::kObjectType);
	
			ECS::ComponentHandle<StaticMeshComponent> staticMeshComp = sceneObject->entity->get<StaticMeshComponent>();
			...
			for(const MeshInfo* meshInfo : staticMeshComp->meshes)
			{
		 	...
		 	ECS::ComponentHandle<TransformComponent> TransComp = sceneObject->entity->get<TransformComponent>();
		 	...
		 	for (int i = 0; i < 3; i++) {
		 		vec3 rot = TransComp->transform.rotation.ToVec();
	...
	for (const auto& entity : currentScene.sceneObjects)
	{
		...
		ECS::ComponentHandle<TransformComponent> transformComp = entity->entity->get<TransformComponent>();
